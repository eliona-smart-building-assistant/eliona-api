...
The BuildingPro Suites REST API enables unified access to the resources and data of an BuildingPro Suites environment.
...
In view of a simplified editing the specification is partitioned into multiple files. The file `buildingpro-suites-api.yaml` combines all the other files grouped by `parameters`, `paths` and `schemas`.

[See BuildingPro Suites APIv2 Documentation](https://api.eliona.io/)
...
Each new version of the BuildingPro Suites REST API specification requires a list of further generation steps. These steps ensure, that the version is available in the API implementation itself and in all dependent modules.
...
To implement the changes made to the API specification, you have to (re-)generate the source code for the API server. The API server is a private repository which covers all the implementation to access an BuildingPro Suites environment. A documentation how to generate the server source code is included in the API server repository.
